...
class VerticalChart extends \Ease\Html\UlTag
{
    ...
    public function __construct(array $axis = [], array $properties = [])
    {
        ...
        if (!empty($axis)) {
            ...
            foreach ($axis as $axe) {
                $axisLi->addItem(new \Ease\Html\DivTag(
                    $axe,
                    ['class' => 'label'],
                ));
            }
     ...
     */
    public function addBar($percent, $amount, $caption, $addClass): void
    {
        $bar = new \Ease\Html\LiTag(
            null,
            ['class' => 'bar '.$addClass, 'style' => 'height: '.$percent.'px;', 'title' => $caption],
        );
        $bar->addItem(new \Ease\Html\DivTag(
            [$percent, new \Ease\Html\SpanTag('%')],
            ['class' => 'percent'],
        ));
